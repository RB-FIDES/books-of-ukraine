...
tryCatch({
  if (requireNamespace("readxl", quietly = TRUE)) {
    ...
    # Basic data type checking and correction
    ua_metadata <- ua_metadata %>%
      mutate(across(everything(), ~if(is.list(.x)) as.character(.x) else .x)) %>%  # Convert list columns to character
      mutate(across(where(is.logical), as.character)) # Convert logical to character
      
    cat("   ✓ Applied basic data type corrections\n")
