# ----------------------------------------------------------------- IMPORT ------------------------------------------------------------------
# last-ellis-claude-sonnet-4.R - Create focused analytical database for human analysts with UA Admin integration
# 
...
# including Ukrainian administrative data (oblasts) integration.
#
# DESIGN PHILOSOPHY:
...
# - UA Admin Integration: Add oblast-level administrative context for enhanced regional analysis
...
# ---- Create UA Admin Wide Tables --------------------------------------------
cat("\n🇺🇦 UKRAINIAN ADMINISTRATIVE DATA INTEGRATION\n")
cat(paste(rep("=", 50), collapse = ""), "\n")

# Create ds_oblast_wide from ua_oblasts_aggregated
if ("ua_oblasts_aggregated" %in% tables) {
	cat("📊 Creating ds_oblast_wide from ua_oblasts_aggregated...\n")
	ua_oblasts_df <- dbReadTable(db, "ua_oblasts_aggregated")
	
	# Transform to wide format for analysis convenience
	# Key metrics as columns for easy comparison across oblasts
	ds_oblast_wide <- ua_oblasts_df %>%
		select(
			oblast_name_en, oblast_code, region_en, region_type,
			total_population, urban_population, urbanization_pct,
			avg_income_per_capita_2022, income_growth_pct,
			oblast_population_density, avg_hromada_size,
			n_hromadas, total_area, avg_travel_time
		) %>%
		arrange(desc(total_population))
	
	append_to_final_db(ds_oblast_wide, "ds_oblast_wide")
	cat("   ✓ ds_oblast_wide created with", nrow(ds_oblast_wide), "oblasts\n")
} else {
	cat("⚠️  ua_oblasts_aggregated not found in Stage 2 database\n")
}
...
# ---- Create UA Admin Long Tables --------------------------------------------

# Create ds_oblast from ua_oblasts_aggregated (long format for statistical analysis)
if ("ua_oblasts_aggregated" %in% tables) {
	cat("📊 Creating ds_oblast from ua_oblasts_aggregated...\n")
	ua_oblasts_df <- dbReadTable(db, "ua_oblasts_aggregated")
	
	# Transform to long format suitable for ggplot2 and statistical modeling
	# Each row represents one oblast-measure combination
	ds_oblast <- ua_oblasts_df %>%
		select(oblast_name_en, oblast_code, region_en, region_type,
			   total_population, urban_population, urbanization_pct,
			   avg_income_per_capita_2022, income_growth_pct,
			   oblast_population_density, avg_hromada_size,
			   n_hromadas, total_area, avg_travel_time) %>%
		tidyr::pivot_longer(
			cols = c(total_population, urban_population, urbanization_pct,
					avg_income_per_capita_2022, income_growth_pct,
					oblast_population_density, avg_hromada_size,
					n_hromadas, total_area, avg_travel_time),
		...
		mutate(
			...
			year = 2022  # Most measures are from 2022
		) %>%
		select(year, category_type, category_value, measure_type, value, 
			   oblast_name_en, oblast_code, region_en, region_type) %>%
		arrange(category_value, measure_type)
	
	append_to_final_db(ds_oblast, "ds_oblast")
	cat("   ✓ ds_oblast created with", nrow(ds_oblast), "rows covering", 
		length(unique(ds_oblast$oblast_name_en)), "oblasts\n")
} else {
	cat("⚠️  ua_oblasts_aggregated not found for ds_oblast creation\n")
}
...
manifest_lines <- c(
  ...
  "",
  "**Generated by:** `manipulation/1-ellis-ua-admin-claude-sonnet-4.R`",
  "**Database:** `books-of-ukraine.sqlite` (Default analytical database)",
  "**Purpose:** Clean, analysis-ready tables optimized for human analysts with UA Admin integration",
  "",
  ...
  "   ↓ 1-ellis-ua-admin-claude-sonnet-4.R processing",
  "TARGET: books-of-ukraine.sqlite (analytical focus + UA Admin)",
  "```",
  "",
  "### **📊 Table Categories**",
  "",
  ...
  "- **NEW**: ds_oblast_wide for oblast-level administrative analysis",
  "",
  "**LONG FORMAT TABLES** (for statistical analysis, modeling):",
  "- Tidy data format for R, Python analysis",
  "- One observation per row",
  "- **NEW**: ds_oblast for oblast-level statistical modeling",
  "",
  ...
  "",
  "**UKRAINIAN ADMINISTRATIVE TABLES** (UA Admin Integration):",
  "- ds_oblast: Long format oblast data for analysis and modeling",
  "- ds_oblast_wide: Wide format oblast data for comparisons and dashboards",
  "- Population, economic, and administrative context for regional analysis",
  "",
  "---",
  "",
  ...
  "| `category_type` | Character | language, theme, territory, purpose, oblast | Data domain/dimension | Faceting, grouping, filtering |",
  "| `category_value` | Character | Domain-specific values | Specific category (e.g., \"Українська\", \"Kyiv\") | Primary categorical analysis |",
  "| `measure_type` | Character | title_count, copy_count, bookstore_count, population metrics | Type of measurement | Metric selection, comparison |",
  "| `value` | Numeric | 0 to domain maximum | Measured quantity | Quantitative analysis, modeling |",
  "",
  "#### **Ukrainian Administrative Columns** (New in UA Admin Integration)",
  "",
  "| **Column** | **Data Type** | **Description** | **Analysis Use** |",
  "|------------|---------------|-----------------|------------------|",
  "| `oblast_name_en` | Character | Oblast name in English | Primary geographic identifier |",
  "| `oblast_code` | Character | Administrative code | Reliable joining key |",
  "| `region_en` | Character | Macro-region (West/Center/East/South) | Regional aggregation |",
  "| `region_type` | Character | Human-friendly region names | Regional analysis |",
  "| `total_population` | Numeric | Oblast population (2022) | Population-based analysis |",
  "| `urbanization_pct` | Numeric | Urban population percentage | Urban/rural analysis |",
  "| `avg_income_per_capita_2022` | Numeric | Average income per capita | Economic analysis |",
  "| `oblast_population_density` | Numeric | Population per km² | Density analysis |",
  "",
  ...
  "---",
  "",
  "## 🇺🇦 **Ukrainian Administrative Integration Notes**",
  "",
  "### **Oblast Data Enhancement**",
  "",
  "The UA Admin integration provides:",
  "- **ds_oblast**: Long format for statistical analysis and modeling",
  "- **ds_oblast_wide**: Wide format for cross-oblast comparisons",
  "- Population, economic, and administrative context for all 25 Ukrainian oblasts",
  "",
  "### **Potential Cross-Analysis Opportunities**",
  "",
  "1. **Regional Publishing Patterns**: Join territory data with oblast administrative data",
  "2. **Economic Correlation**: Correlate publishing activity with income and population",
  "3. **Urban/Rural Analysis**: Use urbanization metrics with publication patterns",
  "4. **Density Analysis**: Analyze publishing density vs population density",
  "",
  "### **Joining Recommendations**",
  "",
  "- **Primary Key**: Use `oblast_name_en` for most joins",
  "- **Fallback Key**: Use `oblast_code` for administrative precision",
  "- **Regional Aggregation**: Use `region_type` for macro-regional analysis",
  "",
  "---",
  ""
)
for (db_path in sqlite_files) {
  ...
  for (tbl in tables) {
    ...
    if (!is.null(df)) {
      ...
      } else if (all(c("oblast_name_en", "region_type") %in% cols) && grepl("oblast", tbl)) {
        desc <- "Ukrainian administrative data: oblast-level context for regional analysis, population, and economic indicators."
...
cat("🇺🇦 Ukrainian administrative data (ds_oblast, ds_oblast_wide) now integrated for enhanced regional analysis\n")
